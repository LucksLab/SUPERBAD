...
def run(protocol: protocol_api.ProtocolContext):
    ...
    #Tells the robot that the 24 well aluminum block with NEST 1.5mL snapcap is on the temperature module
    wellPlate = tempMod.load_labware('opentrons_24_aluminumblock_nest_1.5ml_snapcap')
    ...
    #Uses a for loop to iterate through rows A, B, C, and D of the well plate
    for i in range(4):
        #Sets the row index for the run through the for loop. Starts at index 0 (row A) and ends at index 3 (row D)
        row = wellPlate.rows()[i]
        #Uses a for loop to iterate through columns 1 and 2 of the well plate
        for x in range(2):
            #Aspirates 300ul from well A1 of the tube rack with the p1000 pipette
            p1000.aspirate(300, reservoir['A1'])
            #Dispenses 300ul into the well in the current column of the current row with the p1000 pipette
            p1000.dispense(300, row[x])
    ...
    #Sets the row and column index at 0 for aspirating from the aluminum block well plate
    aluminum_col_index = 0
    aluminum_row_index = 0
    
    #A for loop that iterates through each line of the csv file
    for csv_row in csv_reader:
        
        #Sets the col index of the generic pcr strip well plate
        curr_col = pcr1.columns()[col_index]
        
        #Gets the current row index of the aluminum block well plate
        wellPlate_row = wellPlate.rows()[aluminum_row_index]
        ...
        if (mix_times > 0):
            ...
            p300.mix(mix_times, mix_amount, wellPlate_row[aluminum_col_index])
        ...
        p300.aspirate(20, wellPlate_row[aluminum_col_index])
        ...
        #Determines if all the rows in the current column of the aluminum block well plate have been filled
        #and if so increases the column index by 1 and sets the row index back to 0
        #so that it starts filling in row 1 of the next column next time through the loop
        if (aluminum_row_index > 3):
            aluminum_col_index += 1
            aluminum_row_index = 0
            
        if (aluminum_col_index > 1):
            aluminum_col_index = 0
            aluminum_row_index = 0
